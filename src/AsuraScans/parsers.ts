...
import type { AsuraChaptersPayload, Filters } from "./interfaces/interfaces";
...
export const parseChapters = ($: CheerioAPI, sourceManga: SourceManga): Chapter[] => {
  ...
  const flightPayloads: string[] = scripts.flatMap((script) =>
    Array.from(script.matchAll(/__next_f\.push\(\[1,"([\s\S]*?)"\]\)/g), (m) =>
      decodeUnicodeEscapes(unescapeJsString(m[1])),
    ),
  );

  // console.log(flightPayloads);

  const chapterPayload = flightPayloads.find((p) => p.includes('"chapters":['));
...
export const parseUpdateSection = async (
  ...
  if (page == 1) {
    ...
    for (const item of $("div.w-full", "div.grid.grid-rows-1").toArray()) {
      ...
      const title: string = $(".col-span-9 > .font-medium > a", item).first().text().trim() ?? "";
      let subtitle: string = $(".flex.flex-col .flex-row a", item).first().text().trim() ?? "";
